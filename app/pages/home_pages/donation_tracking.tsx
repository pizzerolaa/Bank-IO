...
const steps = [
    ...
    { title: 'Alimentos recibidos en el banco', icon: Package, date: '21/03/2023' },
...
function DonationTracking({ navigation }) {
    ...
    const [notifications, setNotifications] = React.useState([
        { title: 'Recolección programada', message: 'Tu donación será recogida mañana a las 10:00 AM.' },
        { title: 'En camino', message: 'El equipo de recolección está en camino a tu ubicación.' },
    ]);
                ...
                <Card.Content>
                    ...
                    {steps.map((step, index) => (
                        <View key={index} style={styles.timelineItem}>
                            <View style={[styles.timelineLeft, index <= currentStep && styles.activeTimelineLeft]}>
                                <Avatar.Icon
                                    size={40}
                                    icon={() => <step.icon color={index <= currentStep ? 'white' : theme.colors.surface} />}
                                    style={[styles.timelineIcon, index <= currentStep && styles.activeTimelineIcon]}
                                />
                                {index < steps.length - 1 && <View style={[styles.timelineLine, index < currentStep && styles.activeTimelineLine]}/>}
                            </View>
                            <View style={styles.timelineContent}>
                                <Paragraph style={[styles.timelineTitle, index <= currentStep && styles.activeTimelineTitle]}>{step.title}</Paragraph>
